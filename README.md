...
| 中文显示名 | 规则 ID | MythicMobs ID | 触发环境 |
| --- | --- | --- | --- |
| 满月僵尸骑士 | `fullmoon_zombie_knight` | `FullMoonZombieKnight` | 满月夜晚 / 午夜 |
| 新月影兽 | `newmoon_shadow_beast` | `ShadowBeast` | 新月午夜，黑森林 / 沼泽 / 红树林沼泽 |
| 雷骨袭击者 | `thunder_night_raider` | `StormboneRaider` | 雷雨夜晚 / 午夜 |

你需要在 MythicMobs 中创建这些 mob，或者把 `mythic-mob-id` 改成你已有的内部怪物名。Moonlife 只负责按生态条件调用 MythicMobs 刷怪，不会设置或覆盖实体的 `Display` / `CustomName`；怪物头顶名字请直接在 MythicMobs 配置中控制。
...
| 月相 | 默认功能 |
| --- | --- |
| `NEW_MOON` 新月 | 在 `MIDNIGHT` 且满足群系与环境条件时刷新新月影兽；黑森林、沼泽、红树林沼泽会成为暗影热点；成熟作物有概率产出影尘；击杀新月影兽解锁图鉴和悬赏 |
| `FULL_MOON` 满月 | 在 `NIGHT` / `MIDNIGHT` 刷新满月僵尸骑士；下界 `NETHER_WART` 获得轻微成长和收获加成；成熟作物有概率产出月露种子；可使用满月祭坛启动满月狂潮 |
...
| `NIGHT` 夜晚 | 满月夜会刷新满月僵尸骑士；雷雨夜会刷新雷骨袭击者，同时玩家在野外承受更高危险并获得更高经验/掉落收益 |
| `MIDNIGHT` 午夜 | 新月午夜会刷新新月影兽；满月午夜会刷新满月僵尸骑士；雷雨午夜同样会触发雷骨袭击者和危险状态规则 |
...
| `THUNDER` 雷暴 | 配合 `NIGHT` / `MIDNIGHT` 提高野外危险度，刷新雷骨袭击者，并给玩家更高风险收益状态 |
...
| `%moonlife_spawn_feature%` | 当前刷怪功能摘要，例如 `刷怪:新月影兽 x1-1 权重8` |
...
messages:
  ...
  features:
    - '<prefix><#CBD5E1>新月功能 ✦ 午夜将可能唤出新月影兽。'
```
