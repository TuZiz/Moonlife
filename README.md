...
| 黎明结算 | 黎明按玩家昨夜战斗、收获、探索行为结算经验和材料 |
| 探索与采集悬赏 | 悬赏支持原版怪击杀、成熟作物收获、群系探索 |
| 热点小地点 | 支持出生点半径热点和群系热点，世界会随月相改变探索收益 |
| 月相成就线 | 图鉴条目可触发成就，奖励称号展示品和生态材料 |
| 祭坛收集目标 | 默认提供轻量收集盆，不启动大规模 RPG 事件 |
...
| 月相 | 默认功能 |
| --- | --- |
| `NEW_MOON` 新月 | 在 `MIDNIGHT` 提高新月夜蛛的荒野出现概率；新月成熟作物有概率产出影尘 |
| `WAXING_CRESCENT` 娥眉月 | 野外移动、探索和出生林地热点轻微增强，适合外出踏查 |
| `FIRST_QUARTER` 上弦月 | 地下挖掘获得轻微效率增益，旧矿回声热点开始活跃 |
| `WAXING_GIBBOUS` 盈凸月 | 主世界成熟作物成长和额外收获提升，出生林地热点活跃 |
| `FULL_MOON` 满月 | 在 `NIGHT` / `MIDNIGHT` 提高满月僵尸群出现概率；下界 `NETHER_WART` 获得轻微成长和收获加成 |
| `WANING_GIBBOUS` 亏凸月 | 野外采集、经验和掉落收益轻微上浮，河雾带热点活跃 |
| `LAST_QUARTER` 下弦月 | 地下探索承伤略降，旧矿回声热点活跃 |
| `WANING_CRESCENT` 残月 | 潜行探索获得轻微移速和掉落收益，成熟作物有概率产出苍白纤维 |

### 默认日相功能

| 日相 | 默认功能 |
| --- | --- |
| `DAWN` 黎明 | 不做全服刷屏，按玩家昨夜战斗、收获、探索行为进行个人结算 |
...
- 长期目标：悬赏解锁图鉴，图鉴推进成就线，成就奖励称号展示品，材料可用于月相收集盆
...
| `%moonlife_crop_feature%` | 当前作物功能摘要，例如 `作物:晴天白昼成长 成长x1.35 收获+6.00% 变异+0.50%` |
...
| `%moonlife_buff_feature%` | 当前玩家状态摘要，例如 `状态:黄昏采集者 经验x1.15 掉落x1.12 伤害x1.00 承伤x1.00` |
...
| `%moonlife_hotspot%` | 当前命中的生态热点中文名，没有则为 `无` |
...
| `%moonlife_bounties%` | 当前悬赏完整中文列表 |
| `%moonlife_bounty_lines%` | `%moonlife_bounties%` 的别名 |
| `%moonlife_codex_count%` | 玩家已解锁生态图鉴数量 |
| `%moonlife_codex%` | 玩家生态图鉴与月相成就中文列表 |
| `%moonlife_codex_lines%` | `%moonlife_codex%` 的别名 |
| `%moonlife_achievements%` | `%moonlife_codex%` 的别名 |
...
| `altars.yml` | 定义生态材料、悬赏、热点小地点、成就线、称号展示品和轻量收集目标 |
| `crops.yml` | 只定义作物成长、收获、变异规则 |
| `buffs.yml` | 只定义玩家药水、属性、伤害、经验、掉落规则 |
| `config.yml` | 运行时开关、月相/日相通用设置、性能保护、荒野判断、刷新节流 |
| `moon-phases/*.yml` | 只放每个月相文件，配置该月相启用哪些怪物、祭坛、作物、Buff、热点 |
| `solar-phases/*.yml` | 只放每个日相文件，配置 tick 区间、启用规则和是否公告 |
...
兼容说明：旧的 `spawn-rules.yml`、`crop-rules.yml`、`buff-rules.yml`、`features.yml`、`moon-phases.yml`、`solar-phases.yml` 仍可作为兜底读取；旧版 `settings/config.yml`、`moon-phases/settings.yml`、`solar-phases/settings.yml` 也只作为兼容兜底读取。新默认包不再生成这些旧文件，只要新文件存在，Moonlife 会优先使用新结构。
...
祭坛消耗物、生态材料、作物额外收获都支持自定义物品配置。默认轻生态提供“月相收集盆”作为材料收集目标，不启动大规模 RPG 事件。Moonlife 不使用 NMS，`nbt` 节点实际写入 Bukkit PersistentDataContainer，格式使用 `namespace:key`。
